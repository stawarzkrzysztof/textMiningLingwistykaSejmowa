...
```{r}
#| code-summary: "Miejscowości, w których urodziło się przynajmniej 5 posłów X kadencji"
#| label: miastaUrBarChart
...
df |> 
  ...
  # head(20) |> 
  ggplot() +
  aes(x=reorder(miejsceUr, liczba), y=liczba) +
  geom_col(fill="#56B4E9") +
  geom_text(aes(label = liczba), hjust = -0.35, color="#56B4E9", size = 4) +
  scale_y_continuous(breaks=seq(0, 40, by=2), limits=c(0, 42)) +
  coord_flip() +
  theme(
    axis.title = element_blank(),
    axis.text.y      = element_text(color = "black", margin = margin(r = -17, unit = "pt")),
    axis.text.y.left = element_text(color = "black", margin = margin(r = -17, unit = "pt")),
...
df |>
  ...
  scale_y_continuous(breaks = seq(0, 450, by = 25), limits = c(0, 450)) +
  theme(
    axis.title = element_blank(),
    panel.grid.major.x = element_blank(),
...
deg_bins <- df |>
  filter(czyAktywny, wyksztalcenie == "wyższe", !is.na(najwyzszyStopienNaukowy)) |>
  mutate(
    kategoria = case_when(
      najwyzszyStopienNaukowy == "profesor" ~ "profesor",
      najwyzszyStopienNaukowy == "doktor habilitowany" ~ "doktor habilitowany",
      najwyzszyStopienNaukowy == "doktor" ~ "doktor",
      najwyzszyStopienNaukowy %in% c("magister", "magister inżynier") ~ "II stopień studiów (mgr/mgr inż.)",
      najwyzszyStopienNaukowy %in% c("licencjat", "inżynier") ~ "I stopień studiów (lic./inż.)",
      TRUE ~ "inne/zawodowe"
    ),
    kategoria = factor(
      kategoria,
      levels = c("inne/zawodowe", "I stopień studiów (lic./inż.)", "II stopień studiów (mgr/mgr inż.)", "doktor", "doktor habilitowany", "profesor")
    )
  ) |>
...
levels_deg <- c("profesor","dr hab","dr","II stopień studiów","I stopień studiów","inne/zawodowe")
...
deg_by_club <- df |>
  ...
  mutate(
    kategoria = case_when(
      ...
      najwyzszyStopienNaukowy %in% c("magister", "magister inżynier") ~ "II stopień studiów",
      najwyzszyStopienNaukowy %in% c("licencjat", "inżynier") ~ "I stopień studiów",
      TRUE ~ "inne/zawodowe"
    ),
    kategoria = factor(kategoria, levels = levels_deg)
  ) |>
  count(klub, kategoria, name = "n") |>
  tidyr::complete(klub, kategoria, fill = list(n = 0))

ggplot(deg_by_club, aes(x = kategoria, y = klub, fill = n)) +
...
#TODO: zrobic heatmape z odpowiednimi kolorkami (moze wykladniczy gradient a nie liniowy)
```
